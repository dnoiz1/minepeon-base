<?php
/*
api/settings syncs settings in different files and always returns new state
...
elseif (!empty($_REQUEST['settings'])) {
  ...
  $r['data'] = json_decode(@file_get_contents($configMinepeon), true);
  ...
  if(!empty($newdata)&&is_array($newdata)){
    ...
    file_put_contents($configMinepeon, json_encode($r['data'], JSON_PRETTY_PRINT | JSON_NUMERIC_CHECK));
    $r['info'][]=array('type' => 'success', 'text' => 'MinePeon config saved');
  }
  // Load current settings
  else{
    $r['info'][]=array('type' => 'info', 'text' => 'MinePeon config loaded');
  }
