...
class rrd {
    ...
    public static function gradient($vname=FALSE, $start_color='#0000a0ff', $end_color='#f0f0f0', $label=FALSE, $steps=20, $lower=FALSE){
        ...
        if(preg_match('/^#?([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})/i',$end_color,$matches)){
            ...
            throw new Exception("rrd::". __FUNCTION__ . "() Wrong Color Format: '".$end_color."'");   
        }            
        ...
        if(preg_match('/^([0-9]{1,3})%$/', $lower, $matches)){
            if($matches[1] > 100)
                throw new Exception("rrd::". __FUNCTION__ . "() Lower gradient start > 100% is not allowed: '".$lower."'");
    ...
    public static function area($vname=FALSE, $color=FALSE, $text=FALSE, $stack=FALSE){
        ...
        if($color === FALSE){
            throw new Exception("rrd::". __FUNCTION__ . "() Second Parameter 'color' is missing");   
        }else{
            $line .= $color;
        }
        $line .= ":\"$text\"";
	...
	*/
    ...
    public static function def($vname=FALSE, $rrdfile=FALSE, $ds=FALSE, $cf="AVERAGE"){
        ...
        if($rrdfile === FALSE){
            throw new Exception("rrd::". __FUNCTION__ . "() Second Parameter 'rrdfile' is missing");   
        }
        if($rrdfile === FALSE){
            throw new Exception("rrd::". __FUNCTION__ . "() Third Parameter 'ds' is missing");   
        }
    ...
    public static function cdef($vname=FALSE, $rpn=FALSE){
        ...
        if($rpn === FALSE){
            throw new Exception("rrd::". __FUNCTION__ . "() Second Parameter 'rpn' is missing");   
        }
    ...
    public static function hrule($value=FALSE, $color=FALSE, $text=FALSE){
        $line = "";
        if($value === FALSE){
            throw new Exception("rrd::". __FUNCTION__ ."() First Parameter 'value' is missing");   
        }
        if($color === FALSE){
            throw new Exception("rrd::". __FUNCTION__ . "() Second Parameter 'color' is missing");   
        }
	...
	public static function alerter($vname=FALSE, $label=FALSE, $warning=FALSE, $critical=FALSE, $opacity = 'ff', $unit, $color_green = '#00ff00', $color_btw   = '#ffff00', $color_red   = '#ff0000', $line_col = '#0000ff') {
		...
		if($label === FALSE){
			throw new Exception("rrd::". __FUNCTION__ . "() Second Parameter 'label' is missing");
		}
		if($warning === FALSE){
			throw new Exception("rrd::". __FUNCTION__ . "() Third Parameter 'warning' is missing");
		}
		if($critical === FALSE){
			throw new Exception("rrd::". __FUNCTION__ . "() Fourth Parameter 'critical' is missing");
		}
		$line = "";
	...
	public static function ticker($vname=FALSE, $warning=FALSE, $critical=FALSE, $fraction = -0.05, $opacity = 'ff', $color_green = '#00ff00', $color_btw = '#ffff00', $color_red = '#ff0000') {
		...
		if($warning === FALSE){
			throw new Exception("rrd::". __FUNCTION__ . "() Second Parameter 'warning' is missing");
		}
		if($critical === FALSE){
			throw new Exception("rrd::". __FUNCTION__ . "() Third Parameter 'critical' is missing");
		}
		$line = "";
		$green_vname = "var".substr(sha1(rand()),1,4);
        $btw_vname = "var".substr(sha1(rand()),1,4);
    ...
    public static function debug($data=FALSE){
	    ...
	    var_dump($data);
            ...
            ob_end_clean(); 
            throw new Exception("<pre>".$var_dump."</pre>");
	}
    }
