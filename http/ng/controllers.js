...
.controller('CtrlMain', function($scope,$http,$timeout,$window,$filter) {
  ...
  $scope.sync = function(action,data,alert) {
    ...
    data = data || 'load';
    $http.get('api/settings.php?'+action+'='+angular.toJson(data)).success(function(d){
      if(d.info){
  ...
  $scope.tick = function(once,all) {
    $http.get('api/status.php?'+($scope.settings.userDeveloper?'dev=1&':'')+(all||$scope.status.extra?'all=1':'')).success(function(d){
      if(d.info){
        angular.forEach(d.info, function(v,k) {Alertify.log.create(v.type, v.text);});
      }
      // Update status
...
.controller('CtrlStatus', function($scope,$http) {
  ...
  $scope.graphUpdate = function() {
    $http.get('api/graph.php').success(function(d){
      if(d){
...
.controller('CtrlMiner', function($scope,$http,$timeout) {
  ...
  $scope.minerCompat = function(command,parameter) {
    $http.get('api/minercompat.php').success(function(d){
      if(d.info){
        angular.forEach(d.info, function(v,k) {Alertify.log.create(v.type, v.text);});
      }
      if(d.data.pools){
  ...
  $scope.cgminer = function(command,parameter) {
    ...
    var execute = function(){
      $http.get('api/miner.php?command='+(command || 'summary')+'&parameter='+parameter).success(function(d){
        if(d.info){
          angular.forEach(d.info, function(v,k) {Alertify.log.create(v.type, v.text);});
        }
        $scope.tick();
...
.controller('CtrlBackup', function($scope,$http,$timeout) {
  ...
  $scope.backupLocal = function() {
    var promise = $http.get('api/backup.php?name='+$scope.backupName+'&backup='+angular.toJson($scope.items)).success(function(d){
      if(d.info){
        angular.forEach(d.info, function(v,k) {Alertify.log.create(v.type, v.text);});
      }
  ...
  $scope.exportZip = function(name) {
    ...
    window.location.href='api/backup.php?export='+name;
  };
  ...
  $scope.restore = function() {
    $http.get('api/backup.php?restore='+$scope.backups[$scope.restoring].dir).success(function(d){
      if(d.info){
        angular.forEach(d.info, function(v,k) {Alertify.log.create(v.type, v.text);});
      }
  ...
  $scope.reload = function(wait) {
    wait=wait||0;
    var syncNow = function(){
      $http.get('api/backup.php').success(function(d){
        if(d.data){
